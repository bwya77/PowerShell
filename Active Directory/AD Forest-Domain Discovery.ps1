...
# FSMO holders
    Write-Host "FSMO Role Holders:" -ForegroundColor Green
        ...
        $dInfo | Select-Object InfrastructureMaster, RIDMaster, PDCEmulator | Format-Table -AutoSize
...
# Domain password policy
    Write-Host "Domain password policy:" -ForegroundColor Green
        ...
        $dPwd | Select-Object ComplexityEnabled, LockoutDuration, LockoutThreshold, MaxPasswordAge, `
                MinPasswordAge, MinPasswordLength, PasswordHistoryCount, ReversibleEncryptionEnabled, LockoutObservationWindow | Format-List
...
# AD replication links
    Write-Host "AD relplication links:" -ForegroundColor Green
        Get-ADReplicationSiteLink -Filter * -Properties ReplInterval,Options | Format-Table Name,Cost,ReplInterval,Options,SitesIncluded -AutoSize -Wrap

# AD trusts
    Write-Host "Active Directory Trusts:" -ForegroundColor Green
        Get-ADTrust -Filter * -Properties SelectiveAuthentication | Format-Table Name,Direction,ForestTransitive,IntraForest,SelectiveAuthentication -AutoSize -Wrap
...
Write-Host ""
    ...
    Write-Host "Stale user objects (Accounts that haven't logged in within X days):" -ForegroundColor Green
        ...
        $30 = ($users | Where-Object {$_.lastlogondate -lt $30Days -and $_.enabled -eq $true}).count
        $60 = ($users | Where-Object{$_.lastlogondate -lt $60Days -and $_.enabled -eq $true}).count
        $90 = ($users | Where-Object {$_.lastlogondate -lt $90Days -and $_.enabled -eq $true}).count
...
# OUs with blocked inheritance
    Write-Host "List of OUs with Blocked Inheritance:" -ForegroundColor Green
        Get-ADOrganizationalUnit -SearchBase $rDSE.defaultNamingContext -Filter * | Where-Object {(Get-GPInheritance $_.DistinguishedName).GpoInheritanceBlocked -eq "Yes"} | Sort-Object Name | Format-Table Name,DistinguishedName -AutoSize -Wrap
